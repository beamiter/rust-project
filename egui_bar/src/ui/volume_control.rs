...
impl VolumeControlWindow {
    ...
    pub fn draw(&mut self, ctx: &egui::Context, app_state: &mut AppState) {
        ...
        egui::Window::new("🔊 音量控制")
            ...
            .show(ctx, |ui| {
                ...
                self.draw_content(ui, app_state);
    ...
    fn draw_content(&mut self, ui: &mut egui::Ui, app_state: &mut AppState) {
        // 先获取设备信息，避免后续的借用冲突
        ...
        {
            self.draw_device_controls(ui, device, app_state);
        }
    }
        ...
        if device.has_volume_control {
            ui.horizontal(|ui| {
                ...
                if device.has_switch_control {
                    ...
                    if mute_btn.clicked() {
                        if let Err(e) = app_state.audio_manager.toggle_mute(&device_name) {
