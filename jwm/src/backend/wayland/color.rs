...
impl WaylandColorAllocator {
    pub fn new() -> Self {
        Self { pixel_cache: HashMap::new(), schemes: HashMap::new() }
    }
}
impl ColorAllocator for WaylandColorAllocator {
    ...
    fn free_pixels(&mut self, _pixels: &[Pixel]) -> Result<(), Box<dyn std::error::Error>> { Ok(()) }
    fn set_scheme(&mut self, t: SchemeType, s: ColorScheme) { self.schemes.insert(t, s); }
    fn get_scheme(&self, t: SchemeType) -> Option<ColorScheme> { self.schemes.get(&t).cloned() }
    ...
    fn allocate_schemes_pixels(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        ...
        for s in self.schemes.values() {
            colors.push(s.fg); colors.push(s.bg); colors.push(s.border);
        }
        colors.sort_by_key(|c| c.value);
        colors.dedup();
        for c in colors { let _ = self.ensure_pixel(c)?; }
        Ok(())
    }
    fn free_all_theme_pixels(&mut self) -> Result<(), Box<dyn std::error::Error>> {
        self.pixel_cache.clear(); Ok(())
    }
}
