...
pub struct WaylandWinitBackend {
    ...
    output: Output,
    winit_backend: WinitGraphicsBackend<GlesRenderer>,
...
impl WaylandWinitBackend {
    ...
    fn render_if_needed(&mut self) -> Result<(), BackendError> {
        ...
        // Cursor (top-most). Rendered by us.
        {
            let cursor_pos = self
                .state
                .pointer_location
        ...
        // Layer surfaces above normal windows.
        {
            ...
            for layer in [WlrLayer::Overlay, WlrLayer::Top] {
                for ls in map.layers_on(layer) {
                    ...
                    elements.extend(layer_elements.into_iter().map(WinitRenderElement::Surface));
                }
            }
        }
        ...
        for win in self.state.window_stack.iter().rev() {
            ...
            for (popup_surface, popup_rect) in self.state.popup_rects_for_toplevel(*win) {
                ...
                elements.extend(popup_elements.into_iter().map(WinitRenderElement::Surface));
            }
            ...
            elements.extend(win_elements.into_iter().map(WinitRenderElement::Surface));
    ...
    pub fn new() -> Result<Self, BackendError> {
        ...
        // IMPORTANT: do not override WAYLAND_DISPLAY until after winit has created the host window.
        // winit reads WAYLAND_DISPLAY during init; if we point it at our own newly-created server
        // socket, it may try to connect to itself and no host window will appear.
        let _host_wayland_display = std::env::var_os("WAYLAND_DISPLAY");
        ...
        let _ = winit_backend.renderer().bind_wl_display(&display_handle);
        ...
        // Smithay's winit backend renders into an OpenGL surface with inverted coordinate system.
        // Following Anvil, mark the output as Flipped180 so rendering matches compositor coordinates.
            ...
            event_loop
                ...
                .insert_source(winit_event_loop, move |event, _, state| {
                    match event {
                            ...
                            if !focused {
                                ...
                                if let Some(kbd) = state.seat.get_keyboard() {
                                    if debug_keys {
                                        ...
                                        log::info!(
                                            "[winit] focus_lost: pressed_keys={} smithay_mods={:?} cached_mods=0x{:x}",
                                            pressed.len(),
                                            mods,
                                ...
                                if debug_keys {
                                    if let Some(kbd) = state.seat.get_keyboard() {
                                        ...
                                        let cached_mods = shared.lock().ok().map(|s| s.mods_state);
                                        log::info!(
                                            "[winit] focus_gained: smithay_mods={:?} cached_mods=0x{:x}",
                                            mods,
                                            cached_mods.unwrap_or(0)
                                        );
                                    }
                        ...
                        WinitEvent::Input { 0: event } => {
                            process_input_event_windowed(event, state, &shared, &pending_events);
                ...
                .map_err(|e| BackendError::Message(format!("calloop insert_source(winit) failed: {e}")))?;
        }
        ...
        // We render our own cursor; hide host cursor.
        backend.winit_backend.window().set_cursor_visible(false);
                ...
                if let Some(kbd) = state.seat.get_keyboard() {
                    if let Some((_win, surface, _origin)) = state.surface_under(location) {
                        ...
                        let should_focus = match layer_interactivity {
                            Some(smithay::wayland::shell::wlr_layer::KeyboardInteractivity::None) => false,
                            Some(smithay::wayland::shell::wlr_layer::KeyboardInteractivity::OnDemand) => true,
                            Some(smithay::wayland::shell::wlr_layer::KeyboardInteractivity::Exclusive) => true,
                            None => true,
                        };
